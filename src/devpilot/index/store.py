...
from devpilot.models import Chunk, Dependency, IndexUpdate, ScannedFile, ScanResult, Symbol
...
class IndexFormatError(RuntimeError):
    """The cache exists but is not a compatible DevPilot index."""
...
def cache_root(cache_dir: Path | None = None) -> Path:
    ...
    return (
        Path(user_cache_path("devpilot", appauthor=False, ensure_exists=False)) / "indexes"
...
class RepositoryIndex:
        ...
        try:
            ...
            if version == 0:
                ...
                if application_id != APPLICATION_ID:
                    raise IndexFormatError(
                        f"database at {self._database_path} is not a DevPilot index"
                    )
    ...
    def _create_schema(self) -> None:
        ...
        try:
            ...
            raise IndexFormatError(
                "SQLite FTS5 support is required to create a DevPilot index"
            ) from exc
