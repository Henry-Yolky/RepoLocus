...
def create_app(
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError("install devpilot-codebase[api] to use the HTTP service") from exc
    ...
    def service(path: str) -> tuple[Path, DevPilotService]:
        from devpilot.config import Settings

        repository_root = repository(path)
        return repository_root, DevPilotService(Settings.load(repository_root))
