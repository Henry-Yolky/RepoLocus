...
DevPilot 是一个只读、本地优先的代码库理解工具。它不会执行仓库命令，而是在本地
...
```bash
devpilot ask "请求如何进入核心循环？" --model ollama/qwen3-coder
```

只有回环地址上的 Ollama 被视为本地服务；如果把 Ollama 地址配置到其他主机，DevPilot
会将它按远程供应商处理并要求授权。

云模型调用前，DevPilot 会展示将发送的源码片段、文件列表和估算 Token 数。可用
`--allow-cloud` 仅授权本次调用，或同时使用 `--remember-consent` 为当前仓库记住该供应商。
`devpilot ask ... --follow-up` 可在当前进程内继续追问；输入空行即结束，问答历史不会落盘。
...
- 自托管 API：安装 `api` 可选依赖后运行 `devpilot serve`。
...
DevPilot 不修改业务代码、不执行测试或构建脚本、不提交 Git、不自动创建 PR。仓库内的
